...
class OptimizerOnInit
{
    public function __construct()
    {
        ...
        add_action( 'wp_ajax_two_setFlowIdNotificationId', array( '\TenWebOptimizer\OptimizerAdmin', 'setFlowIdNotificationId' ));
        add_action( 'elementor/editor/before_enqueue_scripts', array( $this, 'two_enqueue_editor_scripts' ) );
        /*
         * Fires before updating an DIVI ePanel option in the database.
         */
        add_action( 'et_epanel_update_option', array( $this, 'maybeRegenerateCCSS' ), 10, 2 );
    }
    ...
    public function two_enqueue_editor_scripts() {
        wp_enqueue_script( 'two-elementor-editor-scripts',
            TENWEB_SO_URL . '/assets/js/two_elementor_editor.js',
            array('jquery'),
            TENWEB_SO_VERSION
        );
        wp_localize_script('two-elementor-editor-scripts', 'two_elementor_vars', array(
    ...
    public function maybeRegenerateCCSS( $et_option_name, $et_option_new_value ) {
        ...
        update_option(TW_OPTIMIZE_PREFIX . '_old_divi_custom_css', $oldCustomCSS, false );
        $old_value = et_get_option( $et_option_name, array() );
        $diviOptionNames = array(
            'divi_dynamic_css',
            'divi_critical_css',
